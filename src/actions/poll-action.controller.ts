...
export class PollActionController extends BaseDiscordActionController {
  ...
  protected async handle(
    ...
    //) {
    //  // CREATES THE MODAL WHEN '/POLL' SLASH COMMAND IS EXECUTED
    //  const data = new ModalBuilder()
    //    .setTitle('Create a poll')  //TITLE OF THE MODAL
    //    .setCustomId('poll:modal:modal')
    //    //DATA.COMPONENTS CONTAINS THE TWO TEXTBOXES
    //    .addComponents(
    //      new ActionRowBuilder<ModalActionRowComponentBuilder>().addComponents(
    //        //NEW TEXTBOX FOR POLL DESC.
    ...
    //    )
    //    .toJSON();
    //    //RETURNING IS WHAT PRINTS OUT THE BUTTON
    //  return {
    ...
    //  //interaction.data.components[0] = TEXTBOX FOR POLL DESC.
    //  //interaction.data.components[1] = TEXTBOX FOR POLL OPTIONS
    ...
    //  // choices splits each option by newline   
    //  const choices = options.split('\n');
    //  console.log(choices);
    //  const poll = await this.ListAPI.createPoll(description, choices);
    ...
    //      const allVotes = await this.ListAPI.getAllVotesOnPoll(
    //        vote[1],
    //        0,
    //        1000,
    //      );
    ...
    //        await this.ListAPI.removeVote(existingVote.id);
    //      }
    //
    //      voteCreated = await this.ListAPI.createVote({
    //        poll_id: vote[1],
    //        option_id: vote[2],  //why is there only 1 option id  <------- ASK
    //        identifier: interaction.member!.user.id,
    //      });
    //    }
    //    const poll = await this.ListAPI.getPoll(vote[1]); // isnt pollid vote[1]? <--------- ASK
